...
async function sendConfirmationPrompt(to, entries) {
    await whatsapp_client_1.whatsappClient.sendButtons(to, `I found ${entries.length} income item(s):\n${formatEntries(entries)}\n\nConfirm to save?`, [
        ...
        if (entries.length === 0 && mediaPayload && (mediaPayload.type === 'image' || mediaPayload.type === 'audio')) {
            try {
                ...
                if (entries.length === 0 && extracted.hintDescription) {
                    ...
                    await whatsapp_client_1.whatsappClient.sendText(from, `I heard "${extracted.hintDescription}". What is the amount? Reply like: ${extracted.hintDescription} 200`);
                    return;
                }
            }
            catch (err) {
