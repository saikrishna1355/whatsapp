...
function drawSummaryTable(doc: PDFKit.PDFDocument, totalIncome: number, totalExpense: number, profit: number): void {
...
function drawDataTable(doc: PDFKit.PDFDocument, data: ReportEntry[]): void {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    doc.rect(LEFT, y, TABLE_WIDTH, ROW_HEIGHT).fill(bg).stroke('#cccccc');
    doc.fillColor('#000000');
    doc.text(formatDate(data[i].date), colDate, y + 7);
